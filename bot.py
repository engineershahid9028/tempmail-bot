...
from db import init_db, db, User, Email, Inbox
        ...
        while True:
            try:
                for m in PROVIDER.messages(token):
                    ...
                    sender=mail["from"]["address"]; subject=mail["subject"]; body=mail.get("text","")
                    s=db(); s.add(Inbox(email_id=email_id, sender=sender, subject=subject, body=body)); s.commit(); s.close()
                ...
                time.sleep(10)
    t=threading.Thread(target=loop, daemon=True); listeners[email_id]=t; t.start()
...
def newmail(call):
    ...
    reset_quota(u)
    if u.daily_quota<=0:
        bot.send_message(call.message.chat.id, "❌ Daily quota reached", reply_markup=main_menu()); return
    email, token = PROVIDER.create()
    s=db(); e=Email(user_id=u.id, email=email, token=token); s.add(e); s.commit(); s.refresh(e); s.close()
    u.daily_quota-=1; save_user(u)
...
def reuse(call):
    u=get_user(call.message.chat.id)
    if u.plan!='premium':
        bot.send_message(call.message.chat.id, "❌ Premium only", reply_markup=main_menu()); return
    s=db(); e=s.query(Email).filter(Email.user_id==u.id).order_by(Email.id.desc()).first(); s.close()
    if not e:
        bot.send_message(call.message.chat.id, "No previous email", reply_markup=main_menu()); return
...
def inbox(call):
    ...
    s.close()
    if not rows:
        bot.send_message(call.message.chat.id, "📭 Inbox empty", reply_markup=main_menu()); return
    text="📥 Your Inbox\n\n"
    for r,e in rows:
...
def pay(msg):
    try:
        ...
        bot.send_message(msg.chat.id, "✅ Payment submitted. Admin will review.")
        if ADMIN_ID:
            bot.send_message(ADMIN_ID, f"💳 New payment #{p.id} from {u.telegram_id}\n{txid} {amount} {cur}")
    except:
